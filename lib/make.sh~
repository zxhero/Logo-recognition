...
if [ -d "$CUDA_PATH" ]; then
	...
	g++ -std=c++11 -shared $i -o $o roi_pooling.so roi_pooling_op.cc -D_GLIBCXX_USE_CXX11_ABI=0 -O2\
	roi_pooling_op.cu.o -I$TF_INC -I$TF_INC/external/nsync/public -L$TF_LIB -ltensorflow_framework -D 	  GOOGLE_CUDA=1 \
	-fPIC $CXXFLAGS -lcudart -L $CUDA_PATH/lib64
else
	g++ -std=c++11 -shared $i -o $o roi_pooling.so roi_pooling_op.cc -D_GLIBCXX_USE_CXX11_ABI=0 -O2\
		-I$TF_INC -I$TF_INC/external/nsync/public -L$TF_LIB -ltensorflow_framework -fPIC $CXXFLAGS 
fi
